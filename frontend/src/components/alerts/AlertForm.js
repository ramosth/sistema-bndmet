...
export default function AlertForm({
  ...
  // Função para gerar mensagem com dados do sensor
...
⚠️ Risco Integrado: ${alerta.riscoIntegrado || 'N/A'}%
📊 Nível de Alerta: ${alerta.nivelAlerta || 'N/A'}

💧 PRECIPITAÇÃO:
• Próximas 24h: ${alerta.precipitacaoPrevisao24h || 'N/A'}mm
• Últimas 24h: ${alerta.precipitacao24h || 'N/A'}mm
...
ATENÇÃO: Esta situação requer resposta imediata das equipes responsáveis. Entre em contato com a coordenação técnica URGENTEMENTE.`,
  ...
  useEffect(() => {
    if (alertaOrigem) {
      ...
      // Determinar nível de criticidade baseado no alerta
  ...
  const handleTemplateSelect = (tipo) => {
    ...
    if (template) {
      ...
      // Ajustar nível de criticidade baseado no template
      if (tipo === "informativo") {
        setValue("nivelCriticidade", "baixo");
      } else if (tipo === "atencao") {
        setValue("nivelCriticidade", "medio");
      } else if (tipo === "critico") {
        setValue("nivelCriticidade", "critico");
      }
  ...
  const onSubmit = async (data) => {
    const alertData = {
      ...
      dadosSensor: alertaOrigem ? {
        ...
        precipitacaoPrevisao24h: alertaOrigem.precipitacaoPrevisao24h,
  ...
  const getRecipientCount = (tipo) => {
    const counts = {
      ...
      basico: "Apenas usuários básicos",
    };
    return counts[tipo] || "";
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ padding: 0 }}>
        ...
        <div>
          <div
            style={{ 
              fontWeight: "600", 
              fontSize: "14px", 
              color: alertaOrigem ? "#8b6e00" : "#0369a1" 
            }}
          >
          ...
          <div style={{ 
            fontSize: "12px", 
            color: alertaOrigem ? "#a67c00" : "#0284c7", 
            marginTop: "2px" 
          }}>
            {alertaOrigem
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        {errors.titulo && (
          <span
            style={{ color: "var(--red-500)", fontSize: "12px", marginTop: "4px", display: "block" }}
          >
      ...
      {!alertaOrigem && (
        <div style={{ marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {Object.entries(templatesMensagem).map(([key, template]) => (
              <Button
                key={key}
                type="button"
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        <textarea
          ...
          style={{
            ...
            lineHeight: "1.4",
            resize: "vertical",
            minHeight: alertaOrigem ? "300px" : "120px",
          }}
        />
        {errors.mensagem && (
          <span
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <label
              style={{
                ...
                fontSize: "13px",
                fontWeight: "500",
                backgroundColor: selectedChannels.includes("email")
                  ? "var(--blue-50)"
                  : "transparent",
                color: selectedChannels.includes("email")
                  ? "var(--blue-700)"
                  : "var(--gray-600)",
              }}
            >
            ...
            <label
              style={{
                ...
                fontSize: "13px",
                fontWeight: "500",
                backgroundColor: selectedChannels.includes("sms")
                  ? "var(--green-50)"
                  : "transparent",
                color: selectedChannels.includes("sms")
                  ? "var(--green-700)"
                  : "var(--gray-600)",
                opacity: 0.6,
              }}
