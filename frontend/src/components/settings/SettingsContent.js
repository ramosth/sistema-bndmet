...
export default function SettingsContent() {
  ...
  const onProfileSubmit = async (data) => {
    setLoading(true);
    try {
      // Preparar dados para API - seguindo estrutura do backend
      const userData = {
        nome: data.nome.trim(),
        email: data.email.toLowerCase().trim(),
        perfil: user.perfil, // Manter perfil existente
      };

      // Chamar API do backend
      const response = await userService.updateAdminUser(user.id, userData);

      // Verificar se a resposta foi bem-sucedida
      if (response.success) {
        // Atualizar dados no contexto local com os dados retornados da API
    ...
    try {
      ...
      switch (action) {
        case "cleanup-tokens":
          ...
          toast.success(
            `${response.data.tokensLimpos} tokens expirados removidos`
          );
          break;
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                Esta operação irá executar uma limpeza completa do sistema,
                incluindo tokens expirados, sessões antigas e logs antigos. O
